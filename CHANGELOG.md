...
- **SDR MJPEG encode on modern ffmpeg** — JPEG thumb extraction always uses `-strict unofficial` + `yuvj420p` (not only when HDR tonemap runs). Fixes last-tile failures like `Non full-range YUV is non-standard` on 1-frame fps batches for H.264 remuxes.

## [7.1.3] - 2026-07-22

### Removed

- **Estuary (stock) skin support** — stock Kodi Estuary (`skin.estuary`) cannot be edited, so the dedicated snippet, skin profile, and settings option are removed. Use **Estuary Mod v2** (or another editable skin) for trickplay overlays.
...
- **Skin install result modal** — restored the pre-6.3.5 flow: after install/restore, show `Dialog().ok` with the per-skin summary immediately (the path that actually displayed the modal). Deferred AlarmClock/`skin_result` approaches removed. `ReloadSkin` is armed during install again, so the old OK-vs-reload race may return.
...
- **Skin snippet install crash on summary OK** — `ReloadSkin` was armed during install (2s alarm) while the result OK dialog was still open, so dismissing OK raced the reload. Reload is now scheduled only after the summary dialog closes (3s defer).
