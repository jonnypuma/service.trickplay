...
- **Frame extraction mode** — **Accurate** (slow, frame-accurate), **Fast** (default), or **Fast (batch seeks)** (several seeks per ffmpeg process with Fast fallback; biggest gains on local SDR; legacy setting `experimental` maps here)
